...
RiboseQC_analysis<-function(annotation_file,bam_files,read_subset=TRUE,readlength_choice_method="max_coverage",genome_seq=NULL,stranded = TRUE ,
														normalize_cov=TRUE,chunk_size=5000000L,write_tmp_files=TRUE,dest_names=NA,rescue_all_rls=FALSE,fast_mode=TRUE,create_report=TRUE,sample_names=NA,report_file=NA,extended_report=FALSE,pdf_plots=TRUE,offsets_df=NULL){
    ...
    if(length(sample_names)==1){
        if(is.na(sample_names)){sample_names=paste("sample",seq_along(bam_files),sep="")}
    }
    ...
    for(bammo in seq_along(bam_files)){
        ...
        for(comp in c("nucl",circs)){
            ...
            if(read_subset[bammo]==TRUE){
                rl_ok<-names(cuma[ seq_len(which(cuma>99)[1]) ])
                #rl_ok<-as.character(seq(sort(as.numeric(rl_ok),decreasing = F)[1],sort(as.numeric(rl_ok),decreasing = TRUE)[1]))
            ...
            for(len in c("all",names(ps_comp))){
                ...
                ps_win_5<-DataFrame(t(sapply(cov_5,function(x){as.vector(x)[c(seq_len(25),(length(x)-25):(length(x)-1))]})))
                ...
                ps_win_cds<-DataFrame(t(sapply(cov_cds,function(x){
                    ...
                    as.vector(x)[c(seq_len(33),(mid-17):(mid+15),(length(x)-32):length(x))]
                })))
                tls <- cbind(ps_tiles_5, ps_tiles_cds, ps_tiles_3)
                colnames(tls) <-
                  c(
                    paste("5_UTR", seq_along(ps_tiles_5[1, ]), sep = "_"),
                    paste("CDS", seq_along(ps_tiles_cds[1, ]), sep = "_"),
                    paste("3_UTR", seq_along(ps_tiles_3[1, ]), sep = "_")
                  )
                nts <- cbind(ps_win_5, ps_win_cds, ps_win_3)
                colnames(nts) <-
                  c(
                    paste("5_UTR", seq_along(ps_win_5[1, ]), sep = "_"),
                    paste("CDS", seq_along(ps_win_cds[1, ]), sep = "_"),
                    paste("3_UTR", seq_along(ps_win_3[1, ]), sep = "_")
                  )
                #DataFrameList?
                ps_tiles[[len]]<-tls
                ps_win[[len]]<-nts
            ...
            for(comp in names(rl_cutoffs_comp)){
                ...
                for(i in seq_along(resul$read_length)){
                    ...
                    if(length(ok_reads)>0){
                        ...
                        mapqokay <- mcols(ok_reads)$mapq>50 & grepl(x=mcols(ok_reads)$MD,pattern="\\W|.{3,}")
                        length(mapqokay)
        ...
        if(!normalize_cov[bammo]){
            export(P_sites_stats$coverage_all_plus,con=paste(dest_name,"_coverage_plus.bedgraph",sep = ""))
            export(P_sites_stats$coverage_all_min,con=paste(dest_name,"_coverage_minus.bedgraph",sep = ""))
            export(P_sites_stats$coverage_uniq_plus,con=paste(dest_name,"_coverage_uniq_plus.bedgraph",sep = ""))
            export(P_sites_stats$coverage_uniq_min,con=paste(dest_name,"_coverage_uniq_minus.bedgraph",sep = ""))
        }
        ...
        rm(res_all,read_stats,profiles_fivepr,selection_cutoffs,
