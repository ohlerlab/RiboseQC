# RiboseQC, a comprehensive Ribo-seq quality control tool Authors: Lorenzo
# Calviello (calviello.l.bio@gmail.com) Dominique Sydow
# (dominique.sydow@posteo.de) Dermott Harnett (Dermot.Harnett@mdc-berlin.de) Uwe
# Ohler (Uwe.Ohler@mdc-berlin.de) This software is free software: you can
...
# copy of the GNU General Public License along with this software. If not, see
...
create_html_report <- function(input_files, input_sample_names, output_file, extended = FALSE) {
    ...
    input_files <- paste(normalizePath(dirname(input_files)), basename(input_files), 
        sep = "/")
    output_file <- paste(normalizePath(dirname(output_file)), basename(output_file), 
        sep = "/")
    
    # get path to RMarkdown file (to be rendered)
    rmd_path <- paste(system.file(package = "RiboseQC", mustWork = TRUE), "/rmd/riboseqc_template.Rmd", 
        sep = "")
    if (extended) {
        rmd_path <- paste(system.file(package = "RiboseQC", mustWork = TRUE), "/rmd/riboseqc_template_full.Rmd", 
            sep = "")
    }
...
create_pdfs_from_rds_objects <- function(output_rds_path) {
    ...
    for (rds_file in rds_files) {
        ...
        # some objects can be directly printed, others need as_ggplot
        # (arrangeGrob-derived objects)
        if (class(g)[2] == "ggplot") {
            pdf(paste0(output_rds_path, "../pdf/", rds_file, ".pdf"), width = format[1], 
                height = format[2])
            print(g)
            dev.off()
        } else {
...
plot_read_biotype_dist_1 <- function(pos, sample, output_rds_path = "") {
    ...
    g2 <- ggplot(data = pos.m, aes(x = biotype, y = value, fill = variable)) + geom_bar(stat = "identity", 
        position = position_stack(reverse = TRUE)) + labs(x = "", y = "read count\n") + 
        scale_fill_brewer(palette = "Set2") + theme_minimal() + theme(plot.margin = margin(0.5, 
        0.5, 0.5, 0.5, "cm"), panel.background = element_blank(), axis.ticks = element_blank(), 
        axis.text.x = element_text(angle = 90, hjust = 1, size = 14), axis.text.y = element_text(size = 14), 
        axis.title = element_text(size = 14), legend.position = "bottom", legend.text = element_text(size = 14), 
    ...
    if (!output_rds_path == "") {
        ...
        saveRDS(list(g2, c(10, 5)), file = file_path, ascii = FALSE, version = NULL, 
...
plot_read_biotype_dist_2 <- function(rdata_list, output_rds_path = "") {
    ...
    # read count
    g1 <- ggplot(data = data.m, aes(x = L1, y = value, fill = Var1)) + geom_bar(stat = "identity", 
        position = position_stack(reverse = TRUE)) + scale_fill_viridis(discrete = TRUE) + 
        labs(x = "\nsample", y = "read count\n") + facet_grid(Var2 ~ ., scales = "free_y") + 
        theme_minimal() + theme(plot.margin = margin(0.3, 0.3, 0.3, 0.3, "cm"), panel.background = element_blank(), 
        axis.ticks.x = element_blank(), axis.text.x = element_text(angle = 90, hjust = 1, 
            size = 14), axis.text.y = element_text(size = 14), axis.title = element_text(size = 14), 
        legend.text = element_text(size = 14), legend.title = element_blank(), strip.text.y = element_text(size = 14)) + 
        guides(fill = guide_legend(nrow = 1))
    
    # read count percentage
    g2 <- ggplot(data = data.m, aes(x = L1, y = value, fill = Var1)) + geom_bar(stat = "identity", 
        position = position_fill(reverse = TRUE)) + scale_fill_viridis(discrete = TRUE) + 
        labs(x = "\nsample", y = "read count fraction\n") + facet_grid(Var2 ~ ., 
        scales = "free_y") + theme_minimal() + theme(plot.margin = margin(0.3, 0.3, 
        0.3, 0.3, "cm"), panel.background = element_blank(), axis.ticks.x = element_blank(), 
        axis.text.x = element_text(angle = 90, hjust = 1, size = 14), axis.text.y = element_text(size = 14), 
        axis.title = element_text(size = 14), legend.text = element_text(size = 14), 
        legend.title = element_blank(), strip.text.y = element_text(size = 14)) + 
        guides(fill = guide_legend(nrow = 1))
    ...
    if (!output_rds_path == "") {
        ...
        saveRDS(list(gg, c(10, 7)), file = file_path, ascii = FALSE, version = NULL, 
            compress = TRUE, refhook = NULL)
    }
}
...
plot_read_length_dist <- function(rld, sample, output_rds_path = "") {
    ...
    colssi <- alpha(c("blue", "dark red", "forestgreen", rainbow(length(unique(rld.m$Var1)) - 
        3)), alpha = 0.7)
    
    
    
    g1 <- ggplot(data = rld.m, aes(x = Var2, y = value)) + geom_line(aes(colour = Var1, 
        group = Var1), size = lw) + scale_x_continuous(breaks = scales::pretty_breaks(n = 8)) + 
        labs(x = "\nread length", y = "read count\n") + scale_color_manual(values = colssi) + 
        theme_minimal() + theme(plot.margin = margin(0.3, 0.3, 0.3, 0.3, "cm"), axis.text = element_text(size = 14), 
        axis.title = element_text(size = 14), legend.text = element_text(size = 14), 
        legend.title = element_blank())
    ...
    g2 <- ggplot(data = rld_norm.m, aes(x = Var2, y = value)) + geom_line(aes(colour = Var1, 
        group = Var1), size = lw) + scale_x_continuous(breaks = scales::pretty_breaks(n = 8)) + 
    ...
    if (!output_rds_path == "") {
        ...
        saveRDS(list(gg, c(10, 5)), file = file_path, ascii = FALSE, version = NULL, 
            compress = TRUE, refhook = NULL)
    }
...
plot_read_length_dist_by_biotype <- function(reads_summary, sample, output_rds_path = "") {
    ...
    g1 <- ggplot(data = rld.m, aes(x = Var2, y = value)) + geom_line(aes(colour = Var1, 
        ...
        alpha = 0.8) + facet_grid(L1 ~ ., scales = "free_y") + theme_minimal() + 
        theme(plot.margin = margin(0.3, 0.3, 0.3, 0.3, "cm"), axis.text = element_text(size = 14), 
            axis.title = element_text(size = 14), legend.text = element_text(size = 14), 
            legend.title = element_blank(), strip.text.y = element_text(size = 14)) + 
    ...
    if (!output_rds_path == "") {
        ...
        saveRDS(list(gg, c(10, 8)), file = file_path, ascii = FALSE, version = NULL, 
            compress = TRUE, refhook = NULL)
    }
}
...
plot_read_biotype_dist_by_length <- function(reads_summary, sample, output_rds_path = "") {
    ...
    # ggplot(data=rld.m, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat='identity')
    
    
    # read count
    g1 <- ggplot(data = rld.m, aes(x = Var2, y = value, fill = Var1)) + geom_bar(stat = "identity", 
        position = position_stack(reverse = TRUE)) + scale_x_continuous(breaks = scales::pretty_breaks(n = 8)) + 
        scale_fill_viridis(discrete = TRUE) + labs(x = "\nread length (nt)", y = "read count\n") + 
        facet_grid(L1 ~ ., scales = "free_y") + theme_minimal() + theme(plot.margin = margin(0.3, 
        0.3, 0.3, 0.3, "cm"), panel.background = element_blank(), axis.text.x = element_text(angle = 90, 
        vjust = 0.5, size = 14), axis.text.y = element_text(size = 14), axis.title = element_text(size = 14), 
        legend.text = element_text(size = 14), legend.title = element_blank(), strip.text.y = element_text(size = 14)) + 
        guides(fill = guide_legend(nrow = 1))
    
    # read count percentage
    g2 <- ggplot(data = rld.m, aes(x = Var2, y = value, fill = Var1)) + geom_bar(stat = "identity", 
        position = position_fill(reverse = TRUE)) + scale_x_continuous(breaks = scales::pretty_breaks(n = 8)) + 
        scale_fill_viridis(discrete = TRUE) + labs(x = "\nread length (nt)", y = "read count fraction\n") + 
        facet_grid(L1 ~ ., scales = "free_y") + theme_minimal() + theme(plot.margin = margin(0.3, 
        0.3, 0.3, 0.3, "cm"), panel.background = element_blank(), axis.text.x = element_text(angle = 90, 
        vjust = 0.5, size = 14), axis.text.y = element_text(size = 14), axis.title = element_text(size = 14), 
        legend.text = element_text(size = 14), legend.title = element_blank(), legend.position = "bottom", 
        strip.text.y = element_text(size = 14)) + guides(fill = guide_legend(nrow = 1))
...
get_metagene_data <- function(data, profile_type, res, comp) {
    ...
    if (!(comp %in% names(data[[profile_type]][[res]]))) {
        stop("Variable res takes only: ", paste(names(data[[profile_type]][[res]]), 
            collapse = ", "))
    }
    ...
    # we want to make two separate heatmaps for a) single read lenghts (single) and
    ...
    # modified to direct scaling data_zscore <- t(scale(t(data_none), center=TRUE,
    # scale=TRUE)) # scale centers (true) and scales (true) the columns of a numeric
    # matrix, we want to scale per read length > transpose data_none to have rl as
    # column
    ...
    data <- list(none = suppressMessages(melt(t(data_none))), log2 = suppressMessages(melt(t(data_log2))), 
        zscore = suppressMessages(melt(t(data_zscore))))
    ...
    metagene_data <- list(data_bar = data_bar, data_hm_single = data_hm_single, data_hm_all = data_hm_all, 
        profile_type = profile_type, res = res, comp = comp)
...
plot_metagene_hm <- function(metagene_data, scal, sample = "", output_rds_path = "") {
    ...
    g1 <- ggplot(data_hm_all[[scal]], aes(x = Var1, y = Var2)) + geom_tile(aes(fill = value)) + 
        scale_fill_gradient(low = "white", high = "steelblue", na.value = "gray80") + 
        scale_x_continuous(breaks = res_breaks, labels = res_labels) + scale_y_discrete(expand = c(0, 
        0)) + theme(axis.text = element_text(size = 18), axis.title.y = element_text(size = 18, 
        colour = "white"), axis.ticks.y = element_blank(), panel.background = element_blank(), 
        axis.title.x = element_blank(), axis.ticks.x = element_blank(), axis.text.x = element_blank(), 
        legend.position = "top", legend.direction = "horizontal", legend.title = element_text(size = 18), 
        legend.text = element_text(size = 14)) + labs(colour = "r", y = "lg\n", x = res_x_lab) + 
        geom_vline(xintercept = c(51, 150), lty = 3, color = "black") + geom_point(data = data_hm_all[[scal]], 
        aes(size = "NA"), shape = NA, colour = "gray80") + guides(fill = guide_legend(title = paste0("read count (scale: ", 
        scal, ")"), nrow = 1), size = guide_legend(title = ""))
    
    data_hm_single[[scal]]$Var2 <- as.numeric(as.character(data_hm_single[[scal]]$Var2))
    # heatmap for single read lenghts (single)
    g2 <- ggplot(data_hm_single[[scal]], aes(x = Var1, y = Var2)) + geom_tile(aes(fill = value)) + 
        scale_fill_gradient(low = "white", high = "steelblue", na.value = "gray80") + 
        scale_x_continuous(breaks = res_breaks, labels = res_labels) + scale_y_continuous(breaks = scales::pretty_breaks(n = 10)) + 
        # scale_y_discrete(expand=c(0, 0)) +
    theme(axis.text = element_text(size = 18), axis.title = element_text(size = 18), 
        panel.background = element_blank(), legend.position = "bottom", legend.direction = "horizontal", 
        legend.title = element_text(size = 18), legend.text = element_text(size = 14, 
            angle = 45)) + labs(colour = "r", y = "read length (nt)\n", x = res_x_lab) + 
        geom_vline(xintercept = c(51, 150), lty = 3, color = "black") + geom_point(data = data_hm_single[[scal]], 
        aes(size = "NA"), shape = NA, colour = "gray80") + labs(fill = paste0("read count (scale: ", 
        scal, ")")) + guides(size = guide_legend(title = ""))
    
    gg <- arrangeGrob(g1, g2, ncol = 1, heights = c(1, 6))
    
    suppressMessages(if (scal == "zscore") {
        gg <- arrangeGrob(g1 + scale_fill_gradient2(low = "blue", mid = "white", 
            high = "firebrick", na.value = "gray80", guide = "colourbar"), g2 + scale_fill_gradient2(low = "blue", 
            mid = "white", high = "firebrick", na.value = "gray80", guide = "colourbar"), 
    ...
    # save plot as RDS object
    if (!output_rds_path == "") {
        file_path = paste0(output_rds_path, sample, "_", comp, "_", "4_", profile_type, 
            "_", "metagene_", unlist(strsplit(res, "_"))[3], "_", scal)
        saveRDS(list(gg, c(12, 10)), file = file_path, ascii = FALSE, version = NULL, 
            compress = TRUE, refhook = NULL)
    }
...
plot_metagene_bar_rmd <- function(metagene_data, sample = "", output_rds_path = "") {
    ...
    # first, check which read lengths you really want to display: only keep read
    # lengths with a cutoff coverage
    rls_selected = rls
    
    # do this only for the 5' profiles (for P-site profiles, we want to see
    # everything that is there, except empty read lengths) profile_type 1 refers to
    # the 5' profiles!
    if (profile_type == "profiles_fivepr") {
...
plot_metagene_bar <- function(metagene_data, rl, sample = "", output_rds_path = "") {
    ...
    # we need to treat subcodon and binned data differently: subcodon data has frame
    # information, therefore frames should be colored accordingly binned data has NO
    # frame information, therefore colour everything in black, please, dear computer
    ...
    if (grepl("subcodon", res)) {
        ...
        g3 <- ggplot(data = x.m, aes(x = pos, xend = pos, y = 0, yend = value)) + 
            geom_segment(aes(colour = frame, group = frame)) + scale_color_manual(values = c("red", 
            "forestgreen", "blue")) + scale_x_continuous(breaks = res_breaks, labels = res_labels) + 
        ...
        g3 <- ggplot(data = x.m, aes(x = pos, xend = pos, y = 0, yend = value)) + 
            geom_segment(aes(group = frame), colour = "black") + scale_x_continuous(breaks = res_breaks, 
            labels = res_labels) + # scale_y_continuous(expand=c(0, 0)) +
        labs(x = res_x_lab, y = "read count\n") + theme_minimal() + theme(legend.position = "bottom", 
            legend.text = element_text(size = 18), legend.title = element_blank(), 
            axis.text = element_text(size = 18), axis.title = element_text(size = 18))
    }
    ...
    if (!output_rds_path == "") {
        file_path = paste0(output_rds_path, sample, "_", comp, "_", "4_", profile_type, 
            "_", "metagene_", unlist(strsplit(res, "_"))[3], "_", rl)
        saveRDS(list(g3, c(12, 10)), file = file_path, ascii = FALSE, version = NULL, 
            compress = TRUE, refhook = NULL)
    }
}
...
plot_frame_dist_boxplot_rmd <- function(analysis_frame_cutoff, sample = "", output_rds_path = "") {
    ...
    comps <- comps[unlist(lapply(comps, function(x) length(analysis_frame_cutoff[[x]]))) != 
        0]
...
plot_frame_dist_boxplot <- function(analysis_frame_cutoff, comp, sample = "", output_rds_path = "") {
    ...
    # or by fraction of reads in maximal frame data$L1 <- factor(data$L1,
    # levels=c(row.names(res_all$selection_cutoffs$results_choice[[comp]]$data),
    # 'all'))
    
    # from boxplot to stat_summary gg <- ggplot(data, aes(x = L1, y = value,
    # fill=Var2)) + geom_boxplot() + #stat_summary(fun.y = mean,
    # geom='point',colour='magenta', size=2) + scale_x_discrete(name='read length') +
    # scale_y_continuous(name='read distribution between frames') +
    # theme(legend.position = 'bottom', panel.background=element_blank()) + labs(fill
    # = 'frame') + guides(fill=guide_legend(title=''))
    
    gg <- ggplot(data, aes(x = L1, y = value, color = Var2, group = Var2)) + stat_summary(fun.data = mean_se, 
        size = 1.5) + # stat_summary(fun.y = mean, geom='point',colour='magenta', size=2) +
    scale_x_discrete(name = "read length") + scale_color_manual(values = alpha(c("red", 
        "forestgreen", "blue"), alpha = 0.8)) + scale_y_continuous(name = "read distribution between frames", 
        limits = c(0, 1)) + theme(legend.position = "bottom", panel.background = element_blank()) + 
        labs(color = "frame") + guides(fill = guide_legend(title = ""))
    ...
    if (!output_rds_path == "") {
        ...
        saveRDS(list(gg, c(7, 5)), file = file_path, ascii = FALSE, version = NULL, 
            compress = TRUE, refhook = NULL)
    }
}
...
get_rl_and_cutoffs <- function(rdata_list) {
    ...
    for (i in c(seq_along(rdata_list))) {
        ...
        for (comp in comps) {
            d <- as.data.frame(res_all$selection_cutoffs$results_choice[[comp]]$data)[c(3, 
                1, 2, 4, 5)]
            names(d) <- c("read length", "cutoff", "frame preference (%)", "gain_codon?", 
                "gain_new_codons?")
...
get_default_rl_selection <- function(rdata_list) {
    ...
    for (i in seq_along(rdata_list)) {
        ...
        for (j in comps) {
            ...
            rl_choices_default <- rbind(rl_choices_default, c(compartment = j, `read lengths of choice (ordered by quality)` = paste(data_default, 
                collapse = ", ")))
        }
...
get_top50_mapping <- function(rdata_list) {
    ...
    for (i in c(seq_along(rdata_list))) {
        ...
        data <- data[, c("score", "pct", "seqnames", "region", "gene_biotype", "gene", 
            "seq")]
        names(data) <- c("read count", "library fraction (%)", "seqnames", "region", 
            "gene biotype", "gene", "sequence")
...
get_codon_usage_data <- function(data, data_type, comp, rl) {
    # test input parameters for existence
    if ((data_type != "Codon_counts") & (data_type != "P_sites_percodon") & (data_type != 
        "P_sites_percodon_ratio") & (data_type != "A_sites_percodon") & (data_type != 
        "A_sites_percodon_ratio") & (data_type != "E_sites_percodon") & (data_type != 
        "E_sites_percodon_ratio")) {
    ...
    if (!(comp %in% names(data$profiles_P_sites[[data_type]]))) {
        stop("Variable res takes only: ", paste(names(data$profiles_P_sites[[data_type]]), 
            collapse = ", "))
    }
    if (!(rl %in% names(data$profiles_P_sites[[data_type]][[comp]]))) {
        stop("Variable rl takes only: ", paste(names(data[["profiles_P_sites"]][[data_type]][[comp]]), 
            collapse = ", "))
    }
    
    # POSITIONAL CON USAGE
    ...
    gencode <- factor(gencode, levels = c("M", "G", "A", "V", "I", "L", "F", "Y", 
        "W", "C", "P", "S", "T", "N", "Q", "R", "H", "K", "D", "E", "*"))
    
    # add scaling (none, log2, zscore) none
    d_none <- cbind(aa = gencode, codon = codons, d)
    # log2
    d_log2 <- cbind(aa = gencode, codon = codons, log2(d + 1))
    # zscore
    d_zscore <- cbind(aa = gencode, codon = codons, data.frame(scale(d, center = TRUE, 
        scale = TRUE)))
    ...
    codon_usage_data <- list(data_positional = data_positional, data_bulk = d_bulk, 
        data_type = data_type, comp = comp, rl = rl)
...
plot_codon_usage_positional <- function(codon_usage_data, scal, sample = "", output_rds_path = "") {
    ...
    # since ATG contains much more data than the other codons, we will plot two
    # heatmaps: ATG and non-ATG
    ...
    d.m$position <- factor(c(rep("start", nrow(d.m)/3), rep("middle", nrow(d.m)/3), 
        rep("stop", nrow(d.m)/3)), levels = c("start", "middle", "stop"))
    g1 <- ggplot(d.m, aes(x = variable, y = codon)) + geom_tile(aes(fill = value), 
        colour = "darkgrey") + facet_grid(aa ~ position, scales = "free", space = "free") + 
        scale_fill_gradient(low = "white", high = "steelblue", na.value = "gray80") + 
        scale_y_discrete(expand = c(0, 0)) + theme(axis.text.y = element_text(size = 18, 
        family = "mono"), axis.title.y = element_text(size = 18), axis.text.x = element_blank(), 
        axis.title.x = element_blank(), axis.ticks = element_blank(), panel.background = element_blank(), 
        legend.position = "top", legend.direction = "horizontal", legend.title = element_text(size = 18), 
        legend.text = element_text(size = 14), strip.text = element_text(size = 18)) + 
        labs(colour = "r", y = "", x = "\nposition (codon resolution)") + geom_point(data = d.m, 
        aes(size = "NA"), shape = NA, colour = "gray80") + guides(fill = guide_legend(title = paste0(data_type, 
        "(scale: ", scal, ")"), nrow = 1), size = guide_legend(title = ""))
    ...
    d.m$position <- factor(c(rep("start", nrow(d.m)/3), rep("middle", nrow(d.m)/3), 
        rep("stop", nrow(d.m)/3)), levels = c("start", "middle", "stop"))
    g2 <- ggplot(d.m, aes(x = variable, y = codon)) + geom_tile(aes(fill = value), 
        colour = "darkgrey") + facet_grid(aa ~ position, scales = "free", space = "free") + 
        scale_fill_gradient(low = "white", high = "steelblue", na.value = "gray80") + 
        scale_y_discrete(expand = c(0, 0)) + theme(axis.text.x = element_text(size = 18), 
        axis.text.y = element_text(size = 18, family = "mono"), axis.title = element_text(size = 18), 
        ...
        legend.text = element_text(size = 14, angle = 45), strip.text.y = element_text(size = 18), 
        strip.text.x = element_blank()) + labs(colour = "r", y = "", x = "\nposition (codon resolution)") + 
    ...
    # save plot as RDS object
    if (!output_rds_path == "") {
        file_path = paste0(output_rds_path, sample, "_", comp, "_", "7_codonusage_positional_", 
            gsub(" ", "_", data_type), "_", rl, "_", scal)
        saveRDS(list(gg, c(12, 16)), file = file_path, ascii = FALSE, version = NULL, 
            compress = TRUE, refhook = NULL)
    }
...
plot_codon_usage_bulk <- function(codon_usage_data, sample = "", output_rds_path = "") {
    ...
    g1 <- ggplot(d_bulk.m[match("ATG", d_bulk.m$codon), ], aes(x = codon, y = value)) + 
        geom_bar(aes(fill = aa), stat = "identity") + facet_grid(. ~ aa, scales = "free", 
        space = "free") + labs(x = "", y = data_type) + scale_y_continuous(expand = c(0, 
        0)) + scale_fill_manual(values = c("gray")) + theme(axis.text.x = element_text(angle = 90, 
        hjust = 1, vjust = 0.5, size = 16, family = "Courier"), axis.text.y = element_text(size = 16), 
        axis.title.x = element_blank(), axis.title.y = element_text(size = 16), axis.ticks.x = element_blank(), 
        panel.background = element_rect(), panel.border = element_blank(), panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), strip.text = element_text(size = 16), 
        legend.position = "none")
    
    g2 <- ggplot(d_bulk.m[-match("ATG", d_bulk.m$codon), ], aes(x = codon, y = value)) + 
        geom_bar(aes(fill = aa), stat = "identity") + facet_grid(. ~ aa, scales = "free", 
        space = "free") + labs(x = "", y = data_type) + scale_y_continuous(expand = c(0, 
        0)) + scale_colour_hue() + theme(axis.text.x = element_text(angle = 90, hjust = 1, 
        vjust = 0.5, size = 16, family = "Courier"), axis.text.y = element_text(size = 16), 
        axis.title = element_blank(), axis.ticks.x = element_blank(), panel.background = element_rect(), 
    ...
    # save plot as RDS object
    if (!output_rds_path == "") {
        file_path = paste0(output_rds_path, sample, "_", comp, "_", "8_codonusage_bulk_", 
            gsub(" ", "_", data_type), "_", rl)
        saveRDS(list(gg, c(12, 8)), file = file_path, ascii = FALSE, version = NULL, 
            compress = TRUE, refhook = NULL)
    }
...
setMethods(f = "unlist", "GRanges", function(x) {
    return(x)
})

setMethods(f = "unlist", "GAlignments", function(x) {
    return(x)
})
...
choose_readlengths <- function(summary_data, choice = "max_coverage", nt_signals) {
    ...
    for (i in names(summary_data)) {
        ...
        for (cnt in seq_along(rownames(data))) {
            if (cnt > length(rownames(data))) {
                break
            }
            rl <- rownames(data)[cnt]
            cutf <- data$cutoff[cnt]
            sig <- as.matrix(sign[[rl]])
            # sig<-do.call(args = sig[c('5-UTR_1','5-UTR_2','CDS_1','CDS_2','3-UTR_1')],what
            # = cbind.data.frame) sig<-sig[,-c(1,2)]
            cdss <- grep(colnames(sig), pattern = "CDS")
            sig_ctf <- cbind(matrix(0, ncol = cutf, nrow = nrow(sig)), sig[, seq_len(length(sig[1, 
                ]) - cutf)])[, cdss]
            ...
            codon_cov <- codon_cov[!apply(codon_cov, 1, FUN = function(y) {
                sum(is.na(y)) > 0
            }), ]
            ...
            codon_gain <- codon_gain[!apply(codon_gain, 1, FUN = function(y) {
                sum(is.na(y)) > 0
            }), ]
...
get_ps_fromspliceplus <- function(x, cutoff) {
    ...
    if (sum(mores) > 0) {
        ...
        psmores <- GRanges(IRanges(start = stok, width = 1), seqnames = seqnames(x[mores]), 
            strand = strand(x[mores]), seqlengths = seqlengths(x[mores]))
...
get_ps_fromsplicemin <- function(x, cutoff) {
    ...
    if (rangok != 1) {
        ps <- shift(resize(GRanges(rang[rangok], seqnames = seqnames(x), strand = strand(x), 
            seqlengths = seqlengths(x)), width = 1, fix = "start"), shift = -(cutoff - 
            sum(rang[seq_len(rangok - 1)]@width)))
    }
    return(ps)
}
...
load_annotation <- function(path) {
    ...
    if (haspackage) {
        ...
        message(paste0("assigning FaFile_Circ object ", GTF_annotation$genome$path, 
            " to the global workspace as genome_seq"))
        assign("genome_seq", genome_sequence, envir = parent.frame())
    }
...
calc_cutoffs_from_profiles <- function(reads_profile, length_max) {
    ...
    # profok<-do.call(args = reads_profile[c('CDS_1','CDS_2')],what =
    # cbind.data.frame)
    sums <- rowSums(profok)
    ...
    # dists<-c(-24:123) profok<-do.call(args =
    # reads_profile[c('5-UTR_2','CDS_1','CDS_2','3-UTR_1')],what = cbind.data.frame)
    
    # just offset
    profok <- as.matrix(reads_profile)
    Atg <- which(colnames(profok) == "CDS_1")
    colnames(profok) <- as.character(seq(from = -(Atg - 1), length.out = length(profok[1, 
        ]), by = 1))
    ...
    if (length(cutoff_meta) == 0) {
        cutoff_meta = NA
    }
    if (length(cutoff_meta_fra) == 0) {
        cutoff_meta_fra = NA
    }
    
    cutoffs <- c(cutoff, cutoff_fra, cutoff_km, cutoff_km_fra, km_meta, km_meta_fra, 
        cutoff_meta, cutoff_meta_fra)
    names(cutoffs) <- c("absolute_tab", "in_frame_tab", "absolute_km_tab", "in_frame_km_tab", 
        "absolute_km_meta", "in_frame_km_meta", "absolute_meta", "in_frame_meta")
    cutt <- cutoffs[!is.na(cutoffs)]
    final_cutoff <- as.numeric(names(sort(table(cutt[grep(names(cutt), pattern = "frame")]), 
        decreasing = TRUE)[1]))
    ...
    results <- list(final_cutoff = final_cutoff, frames_res = frames_res, cutoffs = cutoffs, 
        frames = frames, profiles_start = profok)
    return(results)
...
prepare_annotation_files <- function(annotation_directory, twobit_file=NULL, gtf_file, 
    scientific_name = "Homo.sapiens", annotation_name = "genc25", export_bed_tables_TxDb = TRUE, 
    forge_BSgenome = TRUE, genome_seq = NULL, circ_chroms = DEFAULT_CIRC_SEQS, create_TxDb = TRUE) {
    DEFAULT_CIRC_SEQS <- unique(c("chrM", "MT", "MtDNA", "mit", "Mito", "mitochondrion", 
        "dmel_mitochondrion_genome", "Pltd", "ChrC", "Pt", "chloroplast", "Chloro", 
        "2micron", "2-micron", "2uM", "Mt", "NC_001879.2", "NC_006581.1", "ChrM", 
    ...
    if (forge_BSgenome) {
        ...
        seed_text <- paste("Package: BSgenome.", scientific_name, ".", annotation_name, 
            ...
            "\n", "Description: Full genome sequences for ", scientific_name, ", ", 
        ...
        seed_dest <- paste(annotation_directory, "/", basename(twobit_file), "_", 
            scientific_name, "_seed", sep = "")
    ...
    if (create_TxDb) {
        ...
        saveDb(annotation, file = paste(annotation_directory, "/", basename(gtf_file), 
            "_TxDb", sep = ""))
        ...
        nc_exons <- reduce(GenomicRanges::setdiff(unlist(exons_ge), reduce(c(unlist(cds_ge), 
            fiveutrs, threeutrs)), ignore.strand = FALSE))
        ...
        for (i in seq_len(length(chunks) - 1)) {
            if (i != (length(chunks) - 1)) {
                mapp <- suppressWarnings(c(mapp, pmapToTranscripts(cds_tx[chunks[i]:(chunks[i + 
                  1] - 1)], transcripts = exsss_cds[chunks[i]:(chunks[i + 1] - 1)])))
            }
            if (i == (length(chunks) - 1)) {
                mapp <- suppressWarnings(c(mapp, pmapToTranscripts(cds_tx[chunks[i]:(chunks[i + 
                  1])], transcripts = exsss_cds[chunks[i]:(chunks[i + 1])])))
            }
        }
        ...
        trann <- unique(mcols(import.gff2(gtf_file, colnames = c("gene_id", "gene_biotype", 
            "gene_type", "gene_name", "gene_symbol", "transcript_id", "transcript_biotype", 
            "transcript_type"))))
        ...
        colnames(trann) <- c("gene_id", "gene_biotype", "gene_name", "transcript_id", 
            "transcript_biotype")
        ...
        g <- unlist(apply(cbind(seq_along(mat_genes), Y = elementNROWS(mat_genes)), 
        ...
        # define genetic codes to use IMPORTANT : modify if needed (e.g. different
        # organelles or species) check ids of GENETIC_CODE_TABLE for more info
        ...
        # Mammalian mito
        translations$genetic_code[rownames(translations) %in% c("chrM", "MT", "MtDNA", 
            "mit", "mitochondrion")] <- "2"
        ...
        # stop codon is the 1st nt, e.g. U of the UAA To-do: update with regards to
        # different organelles, and different annotations
        ...
        mostfr <- by(df[, c("start_pos", "type")], INDICES = df$gene_id, function(x) {
            mfreq <- table(x)
            x$start_pos %in% as.numeric(names(which(mfreq[, 1] == max(mfreq[, 1])))) | 
                x$start_pos %in% as.numeric(names(which(mfreq[, 2] == max(mfreq[, 
                  2]))))
        })
        ...
        # define transcripts as containing frequent start/stop codons or most upstream
        # ones, in relation with 5'UTR length
        ...
        df <- cbind.DataFrame(as.character(seqnames(cds_txscoords)), width(cds_txscoords), 
            start(cds_txscoords), cds_txscoords$mostfreq_stasto, cds_txscoords$gene_id)
        colnames(df) <- c("txid", "cdslen", "utr5len", "var", "gene_id")
        repres_freq <- by(df[, c("txid", "cdslen", "utr5len", "var")], df$gene_id, 
            function(x) {
                x <- x[order(x$var, x$utr5len, x$cdslen, decreasing = TRUE), ]
                x <- x[x$var == max(x$var), ]
                ok <- x$txid[which(x$cdslen == max(x$cdslen) & x$utr5len == max(x$utr5len) & 
                  x$var == max(x$var))][1]
        ...
        df <- cbind.DataFrame(as.character(seqnames(cds_txscoords)), width(cds_txscoords), 
            start(cds_txscoords), cds_txscoords$upstr_stasto, cds_txscoords$gene_id)
        colnames(df) <- c("txid", "cdslen", "utr5len", "var", "gene_id")
        repres_upstr <- by(df[, c("txid", "cdslen", "utr5len", "var")], df$gene_id, 
            function(x) {
        ...
        df <- cbind.DataFrame(as.character(seqnames(cds_txscoords)), width(cds_txscoords), 
            start(cds_txscoords), cds_txscoords$upstr_stasto, cds_txscoords$gene_id)
        colnames(df) <- c("txid", "cdslen", "utr5len", "var", "gene_id")
        repres_len5 <- by(df[, c("txid", "cdslen", "utr5len", "var")], df$gene_id, 
            function(x) {
        ...
        unq_stst$reprentative_5len <- sum(!is.na(match(unq_stst$transcript_id, unlist(as(repres_len5, 
            "CharacterList"))))) > 0
        ...
        GTF_annotation <- list(transcripts_db, txs_gene, ifs, unq_stst, cds_tx, intron_names_tx, 
            cds_gen, exons_tx, nsns, unq_intr, genes, threeutrs, fiveutrs, ncisof, 
            ncrnas, introns, intergenicRegions, trann, cds_txscoords, translations, 
            pkgnm_or_faob, stop_inannot)
        names(GTF_annotation) <- c("txs", "txs_gene", "seqinfo", "start_stop_codons", 
            "cds_txs", "introns_txs", "cds_genes", "exons_txs", "exons_bins", "junctions", 
            "genes", "threeutrs", "fiveutrs", "ncIsof", "ncRNAs", "introns", "intergenicRegions", 
            "trann", "cds_txs_coords", "genetic_codes", "genome", "stop_in_gtf")
        
        # Save as a RData object
        annot_file <- paste(annotation_directory, "/", basename(gtf_file), "_Rannot", 
            sep = "")
        ...
        if (export_bed_tables_TxDb == TRUE) {
            ...
            for (bed_file in c("fiveutrs", "threeutrs", "ncIsof", "ncRNAs", "introns", 
                "cds_txs_coords")) {
                bf <- GTF_annotation[[bed_file]]
                bf_t <- data.frame(chromosome = seqnames(bf), start = start(bf), 
                  end = end(bf), name = rep(".",length(bf)), score = width(bf), strand = strand(bf))
                meccole <- mcols(bf)
                for (mecc in names(meccole)) {
                  if (is(meccole[, mecc], "CharacterList") | is(meccole[, mecc], 
                    "NumericList") | is(meccole[, mecc], "IntegerList")) {
                    meccole[, mecc] <- paste(meccole[, mecc], collapse = ";")
                  }
                }
                bf_t <- cbind.data.frame(bf_t, meccole)
                write.table(bf_t, file = paste(annotation_directory, "/", bed_file, 
                  "_similbed.bed", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE)
                
            }
            
            write.table(GTF_annotation$trann, file = paste(annotation_directory, 
                "/table_gene_tx_IDs", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE)
            seqi <- as.data.frame(GTF_annotation$seqinfo)
            seqi$chromosome <- rownames(seqi)
            write.table(seqi, file = paste(annotation_directory, "/seqinfo", sep = ""), 
                sep = "\t", quote = FALSE, row.names = FALSE)
            
            gen_cod <- as.data.frame(GTF_annotation$genetic_codes)
            gen_cod$chromosome <- rownames(gen_cod)
            write.table(gen_cod, file = paste(annotation_directory, "/genetic_codes", 
