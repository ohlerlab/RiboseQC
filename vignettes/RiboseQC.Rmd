...
library("RiboseQC")
tmp_dir <- tempdir()
tmp_dir <-'/var/folders/5t/j5mzk77n11b5w4q_54rlq97m0000gn/T//RtmpLY1Rhl'
...
Moreover, such function reads a genome file in *.2bit* format to forge a *BSgenome* package for fast and efficient query of genomic sequences (or uses an indexed fasta file). A *.2bit* file can be obtained from a *.fasta* file using the *faToTwoBit* software from UCSC: https://genome.ucsc.edu/goldenpath/help/twoBit.html - http://hgdownload.soe.ucsc.edu/admin/exe/ )
...
RiboseQC_analysis(annotation_file=annot_file,bam_files = file.path(tmp_dir,"test_human_hek.bam"),report_file = file.path(tmp_dir,"test_human_hek.html"),write_tmp_files = F)
```
...
readRDS(file.path(tmp_dir,"test_human_hek.html_plots/rds/sample1_nucl_4_profiles_P_sites_metagene_subcodon_all"))[[1]]

```

Let's now use RiboseQC to analyze different samples at once.
...
prepare_annotation_files(annotation_directory = tmp_dir,
                         ...
                         # genome_seq = file.path(tmp_dir,"test_yeast.2bit"),
...
```{r yeast_2}
load_annotation(file.path(tmp_dir,"test_yeast.gtf_Rannot"))

getSeq(genome_seq,GRanges("II:1-100"))
getSeq(genome_seq,GRanges("Mito:1-100"))

GTF_annotation$cds_txs
GTF_annotation$trann
```

We will now download three files corresponding to sample data from a TCP-seq experiment [4], where the three experiments aim at extracting RNA fragments covered by scanning ribosomes (SSU), elongating ribosomes (RS) and input material (input). RiboseQC analysis is followed by the creation of a report containing results for all the three experiments.
...
bam_filepath_y<-file.path(tmp_dir,c("test_yeast_TCP_input.bam","test_yeast_TCP_RS.bam","test_yeast_TCP_SSU.bam"))

RiboseQC_analysis(annotation_file=file.path(tmp_dir,"test_yeast.gtf_Rannot"),bam_files = bam_filepath_y,fast_mode = TRUE,report_file = file.path(tmp_dir,"test_yeast_TCP.html"),sample_names = c("input","RS","SSU"),dest_names = file.path(tmp_dir,c("input","RS","SSU")),write_tmp_files = F)
```
...
as_ggplot(readRDS(file.path(tmp_dir,"test_yeast_TCP.html_plots/rds/SSU_nucl_4_profiles_fivepr_metagene_subcodon_log2"))[[1]])

as_ggplot(readRDS(file.path(tmp_dir,"test_yeast_TCP.html_plots/rds/RS_nucl_4_profiles_fivepr_metagene_subcodon_log2"))[[1]])
```
...
prepare_annotation_files(annotation_directory = tmp_dir,
                         ...
                         # genome_seq = file.path(tmp_dir,"test_arabidopsis.fa"),
                         gtf_file = file.path(tmp_dir,"test_arabidopsis.gtf.gz"),
                         scientific_name = "arabidopsis.test",
                         annotation_name = "araport11_custom",export_bed_tables_TxDb = TRUE,
                         forge_BSgenome = TRUE,
                         create_TxDb = TRUE,genome_seq = '../test_arabidopsis.fa')
```
...
annotation=file.path(tmp_dir,"test_arabidopsis.gtf.gz_Rannot")
...
RiboseQC_analysis(annotation_file=annotation,bam_files = bam_filepath,fast_mode = TRUE,report_file = file.path(tmp_dir,"test_root_shoots.html"),dest_names = file.path(tmp_dir,c("root","shoots")),sample_names = c("root","shoots"),
                  # genome_seq=here::here('test_arabidopsis'),
                  write_tmp_files = FALSE
                  )
# 
# RiboseQC_analysis(annotation_file=annotation,bam_files = bam_filepath,fast_mode = TRUE,report_file = file.path(tmp_dir,"test_root_shoots.html"),dest_names = c("root2","shoots2"),sample_names = c("root","shoots"),write_tmp_files = FALSE,genome_seq=here::here('test_arabidopsis.fa'))
...
as_ggplot(readRDS(file.path(tmp_dir,"./test_root_shoots.html_plots/rds/shoots_ChrC_7_codonusage_positional_A-sites_per_codon_all_zscore"))[[1]])
```
